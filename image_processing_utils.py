...
def extract_jpg_with_orientation(filename):
    if filename.lower().endswith(".dng"):
        ...
        output_file = os.path.join(
            jpg_directory, filename.lower().replace(".dng", ".jpg")
        )
        ...
        try:
            ...
            with open(output_file, "wb") as outfile:
                result_extract = subprocess.run(
                    cmd_extract, stdout=outfile, stderr=subprocess.PIPE
                )

            # Adjust orientation of the extracted JPG
            if "Error" not in result_extract.stderr.decode():
                result_orientation = subprocess.run(
                    cmd_orientation, stderr=subprocess.PIPE
                )
...
def extract_and_cache_metadata(filename):
    ...
    try:
        ...
        print(
            f"File not found: {source_file}. Ensure the path to Exiftool and the image files are correct."
        )
...
def extract_lens_features(row):
    ...
    features["FocalLength"] = (
        int(focal_length_match.group(1)) if focal_length_match else None
    )
    features["MaxAperture"] = (
        float(max_aperture_match.group(1)) if max_aperture_match else None
    )
    # Use the character immediately after the aperture value for 'Professional'
    features["Professional"] = (
        max_aperture_match.group(2)
        if max_aperture_match and max_aperture_match.group(2)
        else "No"
    )
    features["Version"] = (
        roman_to_arabic(version_match.group(0)) if version_match else 0
    )
